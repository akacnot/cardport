...
export function priceItems(items,products){return items.map(i=>{const p=products.find(p=>p.id===i.id);if(!p||p.archived||p.stock<i.quantity||p.price!==i.expectedPrice)throw problem('failed-precondition','価格・在庫が変わりました。カートを確認してください。');return {id:i.id,name:p.name,quantity:i.quantity,price:p.price}})}
