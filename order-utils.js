export function validateCart(data){
  if(!data||typeof data.requestId!=='string'||!/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/.test(data.requestId)||!Array.isArray(data.items)||!data.items.length||data.items.length>8)throw problem('invalid-argument','1回の注文は1〜8種類までです。');
