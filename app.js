...
function renderAdmin(){if(!adminSignedIn)return;$('#admin-product-count').textContent=`（${PRODUCTS.length}件）`;$('#admin-products').innerHTML=PRODUCTS.map(p=>`<div class="admin-product"><div><strong>${esc(p.name)}</strong><p>${esc(p.game)} · ${yen(p.price)} · 在庫 ${p.stock}</p></div><button class="add" data-edit="${esc(p.id)}">編集</button></div>`).join('');$('#admin-orders').innerHTML=orderList()}
...
function showPending(){$('#pending-banner').hidden=!pending;$('#checkout').textContent=pending?'送信済み注文の結果を確認':'注文を確定する'}
...
async function checkout(){
  ...
  try{
    ...
    if(!pending){
      ...
      if(items.length>8)throw Error('1回の注文は8種類までです。');
    ...
    const result=await api.placeOrder({requestId:pending.requestId,items:pending.items});
    cart={};save();clearPending();$('#cart-dialog').close();$('#order-serial').textContent=result.serial;
    $('#order-summary').innerHTML=`<ul>${result.items.map(i=>`<li>${esc(i.name)} × ${i.quantity}</li>`).join('')}</ul><p class="cart-total">合計（税込） <b>${yen(result.total)}</b></p>`;
...
try{
  ...
  client.connect({
    ...
    config(enabled){ordersEnabled=enabled;renderCart();$('#connection-status').textContent=enabled?'注文を受け付けています。オンライン決済は未対応です。':'商品をご覧いただけます。現在、注文受付は停止中です。'},
